...
Esta app (`@elineas/auth-backend`) es la API del IS. Vive en el monorepo
`elineas-auth` junto al panel de administración (`apps/frontend`), que la consume
...
Copia `apps/backend/.env.example` a `apps/backend/.env.local` y rellena los secretos.
`ALLOWED_ORIGIN`
...
bun run dev:backend    # solo la API; `bun run dev` levanta también el panel
```
