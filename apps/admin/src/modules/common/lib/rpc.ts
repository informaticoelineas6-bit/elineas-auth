...
//
// Los tipos los expone `@elineas/auth-api/rpc` y son SOLO tipos: no arrastran
// nada del runtime del servidor (BD, Redis, better-auth) al bundle del panel.
// Por eso `@elineas/auth-api` está en devDependencies y no en dependencies.
//
...
// caso de `/api/users/me`—. Ver el comentario en `apps/api/src/rpc.ts`.
//
// El prefijo de montaje se declara aquí una única vez por grupo y debe coincidir
// con la tabla de `apps/api/src/routes/index.ts`.
const options = {
